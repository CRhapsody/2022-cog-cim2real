...
class CogEnvDecoder:
    ...
    @staticmethod
    def cal_distance(my_pos, goal_pos):
        return (my_pos[0] - goal_pos[0])**2 + (my_pos[1] - goal_pos[1])**2


    def cal_reward(self, obs, self_pos, goal1, goal2 ,goal3 ,goal4 , goal5, collision_info):
        '''
        根据和目标点A-E的距离计算reward
        goal i: [x_i, y_i, bool(reach or not)]
        collision info :[0, total collsion time]
        obs[1][13]:到达了几个goal, 必须先到A, 再B, 再C ...
        

        self_pos: [xPos, zPos, angle]
        '''
        A_coe = -5;A_reward = 100
        B_coe = -4;B_reward = 200
        C_coe = -3;C_reward = 300
        D_coe = -2;D_reward = 400
        E_coe = -1;E_reward = 500

        # add goal_info of reward
        if obs[1][13] < 0.001:

            reward = A_coe *  CogEnvDecoder.cal_distance(my_pos=self_pos[:-1], goal_pos= goal1[:-1])
        
        elif obs[1][13] >=1 and obs[1][13] <2 :
            reward = A_reward + B_coe *  CogEnvDecoder.cal_distance(my_pos=self_pos[:-1], goal_pos= goal2[:-1])
        
        elif obs[1][13] >=2 and obs[1][13] <3 :
            reward = A_reward + B_reward + C_coe *  CogEnvDecoder.cal_distance(my_pos=self_pos[:-1], goal_pos= goal3[:-1])
        elif obs[1][13] >=3 and obs[1][13] <4 :
            reward = A_reward + B_reward + C_reward + D_coe *  CogEnvDecoder.cal_distance(my_pos=self_pos[:-1], goal_pos= goal4[:-1])

        elif obs[1][13] >=4 and obs[1][13] <5 :
            reward = A_reward + B_reward + C_reward + D_reward + E_coe *  CogEnvDecoder.cal_distance(my_pos=self_pos[:-1], goal_pos= goal5[:-1])

        else:
            reward = A_reward + B_reward + C_reward + D_reward + E_reward
        
        # add collision_info of reward
        reward = reward + collision_info[1] * 20
    ...
    def step(self, action):
        ...
        reward = self.cal_reward(obs, self_pos, goal1, goal2 ,goal3 ,goal4 , goal5, collision_info)
